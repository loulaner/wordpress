...
jQuery(document).ready(function() {
	...
	jQuery('#nr_tabs').change(function() {
		...
		for(i=1;i<=jQuery(this).val();i++){
			var title_content = '<div><label>选项卡 ' +i+ ' 标题: </label> <input type="text" name="title_tab" id="title_tab_'+i+'"></div>'; 
	...
	jQuery('#nr_tabs_accordion').change(function() {
		...
		for(i=1;i<=jQuery(this).val();i++){
			var title_content = '<div><label>选项卡 ' +i+ ' 标题: </label> <input type="text" name="title_tab_acc" id="title_tab_acc_'+i+'"></div>'; 
...
function insertTabs(){
	...
	jQuery('[name="title_tab"]').each(function(i) {   
		...
		if( jQuery.trim(jQuery(this).val()) != '' ){
		 ...
		 tabs = tabs + '[tab title="'+tab_name+'"]添加 '+tab_name+' 内容到这里.[/tab]'; 
		nr_tabs ++;
	});
