<?php
	...
	options::$fields['general']['sim_likes']            = array( 'type' => 'st--button' , 'value' => __( '生成' , 'cosmotheme' ) , 'label' => __( 'Generate random number of Loves for posts' , 'cosmotheme' ) , 'action' => "act.sim_likes( 1 );" , 'hint' => __( 'WARNING! This will reset all current Loves.' , 'cosmotheme' ) );
	options::$fields['general']['reset_likes']			= array( 'type' => 'st--button' , 'value' => __( '重置' , 'cosmotheme' ) , 'label' => __( 'Reset likes' , 'cosmotheme' ) , 'action' =>"act.reset_likes(1);" , 'hint' => __( 'WARNING! This will reset all the likes for all the posts!', 'cosmotheme'  ) );
	...
	options::$fields['general']['fb_app_id_note']       = array( 'type' => 'st--hint' , 'value' => __( '您可以设置Facebook应用程序ID' , 'cosmotheme' ) . ' <a href="admin.php?page=cosmothemes__social">' . __( '在这里' , 'cosmotheme') . '</a> ' );
    ...
    options::$fields[ 'header' ][ 'header_type' ] = array(
        ...
        'labels' => array(
            'centered' => __( '页眉中心' , 'cosmotheme' ),
            'searchbar' => __( '带搜索栏' , 'cosmotheme' ),
            'menu' => __( '带菜单' , 'cosmotheme' ),
            'social' => __( '带社交图标' , 'cosmotheme' )
        )
    );
    ...
    options::$fields[ 'header' ][ 'menu_type' ] = array(
        ...
        'labels' => array(
            'centered'      => __( '中心菜单' , 'cosmotheme' ),
            'text'          => __( '文字菜单' , 'cosmotheme' ),
            'colored'       => __( '彩色菜单' , 'cosmotheme' ),
            'description'   => __( '带描述菜单' , 'cosmotheme' ),
            'buttons'       => __( '按钮菜单' , 'cosmotheme' ),
            'vertical'      => __( '垂直菜单', 'cosmotheme' )
        )
    );

    options::$fields[ 'header' ][ 'menu_color' ] = array(
        'label' => __( 'Color of the menu' , 'cosmotheme' ),
...
*/
    ...
    options::$fields['styling']['background_image']     = array( 'type' => 'st--hint' , 'value' => __( '要设置背景图片去' , 'cosmotheme' ) . ' <a href="themes.php?page=custom-background">' . __( '外观 - 背景'  , 'cosmotheme' ) . '</a>' );
    ...
    */
    ...
    $hint = sprintf(__('当%s 页眉类型为nr. 4 %s启用时链接到下列服务将会出现.','cosmotheme'),'<a href="admin.php?page=cosmothemes__header">','</a>');
    ...
    options::$default[ 'styling' ][ 'box' ]           = 'yes';
    ...
    options::$fields['_sidebar']['save']                = array('type' => 'st--button' , 'value' => '添加侧边栏' , 'action' => "extra.add( '_sidebar' , { 'input' : [ 'sidebar_title_id' , 'sidebar_title'] })" );
    ...
    options::$fields['_tooltip']['save']                = array('type' => 'st--button' , 'value' => __( '添加提示工具' , 'cosmotheme' ) , 'action' => "extra.add( '_tooltip' , { 'input' : [ 'tooltip_id' , 'tooltip_title' , 'tooltip_top' , 'tooltip_left', 'tooltip_res_posts' ] , 'textarea' : 'tooltip_description' , 'select' : ['tooltip_type' , 'tooltip_res_type' ,  'tooltip_res_pages' ] })" );
