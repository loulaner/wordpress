            ...
            if( isset( $struct[ 'actions' ] ) ){
                foreach( $struct[ 'actions' ] as $index => $action ){
                    ...
                    if( $action == 'edit_update' ){
                        $result .= '<a class="edit-action" href="' . self::edit_action( $group , $id ) . '">编辑</a> ';
                        $result .= '<a class="update-action  hidden" href="' . self::update_action( $group , $id ) . '">更新</a> ';
                    }

                    if( $action == 'delete' ) {
                        $result .= '<a class="delete-action" href="' . self::delete_action( $group , $id ) . '">删除</a> ';
                    }
                }
            }else{
                $result .= '<a class="edit-action" href="' . self::edit_action( $group , $id ) . '">编辑</a> ';
