...
if(!class_exists('templ_locationmap'))
{
	class templ_locationmap extends WP_Widget {
		...
		//Constructor
			$widget_ops = array('classname' => 'widget location_map', 'description' => apply_filters('templ_googlemap_widget_desc_filter','输入你的位置或在地图上显示地址的经纬度'));		
			$this->WP_Widget('widget_location_map', apply_filters('templ_googlemap_widget_title_filter','T &rarr; 谷歌地图位置'), $widget_ops);
		}
	...
	<?php }?>
    <?php if($address){?>
    <span class="get_direction"><a href="http://ditu.google.cn/maps?f=d&amp;dirflg=d&amp;saddr=<?php echo $address;?>" target="_blank"><?php echo apply_filters('templ_googlemap_widget_get_direction_filter',__('Get Direction on Map &raquo;','templatic'));?></a></span>
    <?php }?>
	</div>
		<?php
