...
class templ_testimonials_widget extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => 'widget testimonials', 'description' => apply_filters('templ_testimonial_widget_desc_filter','会员证明小工具会显示客户证明到你的网站') );		
		$this->WP_Widget('testimonials_widget',apply_filters('templ_testimonial_widget_title_filter','T &rarr; 会员证明'), $widget_ops);
	}
...
function slideSwitch() {
    var $active = jQuery('#testimonials blockquote.active');

    if ( $active.length == 0 ) $active = $('#testimonials blockquote:last');

    // use this to pull the images in the order they appear in the markup
    var $next =  $active.next().length ? $active.next()
        : jQuery('#testimonials blockquote:first');
