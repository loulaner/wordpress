      ...
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          ...
          $('.select').each(function () {
            $(this).prev('span').text('-- 选择 --') 
          });
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>主题选项已删除</div>');
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>主题选项不能删除</div>');
        }
      });
      return false;
    },
    import_data: function (e) {
      ...
      $.post(ajaxurl, d, function (r) {
        if (r != -1) {
          ...
          $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>你的主题选项数据已成功导入.</div>');
        } else {
          $('.ajax-message').ajaxMessage('<div class="message warning"><span>&nbsp;</span>主题选项数据无法导入.</div>');
        }
      });
      return false;
    },
    remove: function (b) {
      ...
      $.post(ajaxurl, d, function (r) {
        if (r) {
          if (r == 'removed') {
            $("#option-" + c).remove();
            $('.ajax-message').ajaxMessage('<div class="message"><span>&nbsp;</span>选项已删除.</div>');
          } else {
      ...
      $('.select').live('change', function () {
        ...
        if (temp_select == 'Heading') {
          ...
          if (temp_select == 'Textarea') {
            $('.regular').hide();
            $('.alternative').show().html('<strong>行数:</strong> 在文本区域输入行数.');
            $('.option-desc', '#edit-'+c).show();
            $('.option-options', '#edit-'+c).show();
          } else if (
            ...
            $('.regular').hide();
            $('.alternative').show().html('<strong>文章类型:</strong> Enter your custom post_type.');
            $('.option-desc', '#edit-'+c).show();
            $('.option-options', '#edit-'+c).show();
          } else {
            $('.option-desc', '#edit-'+c).show();
