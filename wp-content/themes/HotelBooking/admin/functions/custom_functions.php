...
function templ_seo_meta_content()
{
	global $room_type;
	if($room_type != '') {
		$meta_data = get_meta_keyword($room_type);
		$description = stripslashes($meta_data['meta_description']);
		$keywords = stripslashes($meta_data['meta_keyword']);	
	} else { 
...
function templ_seo_title() {
	...
	{
		global $page, $paged,$room_type;
		$sep = " | "; # delimiter
		$newtitle = get_bloginfo('name'); # default title
		
		if($room_type != '') {
			$newtitle = get_meta_title_room($room_type);
...
function get_meta_title_room($room){
global $wpdb;
	$room_type_table = $wpdb->prefix . "room_type_master";
	$meta_title_sql = mysql_query("select meta_title from $room_type_table where room_type_id = '".$room."'");
	$meta_title_res = mysql_fetch_array($meta_title_sql);
	return $meta_title_res['meta_title'];
}
function get_meta_keyword($room_type_id)
{
	global $wpdb;
	$room_type_table = $wpdb->prefix . "room_type_master";
	$meta_content_sql = mysql_query("select meta_title,meta_description,meta_keyword from $room_type_table where room_type_id = '".$room_type_id."'");
	$meta_res = mysql_fetch_array($meta_content_sql);
	return $meta_res;
}
?>